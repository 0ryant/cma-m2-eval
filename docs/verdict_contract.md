...
- **TIED_AT_CEILING rule (pre-registered):** Under disjoint action
  supports, between-state JS saturates at ln(2) by construction.
  When `abs(m2_between - ln2) < 0.005 AND abs(lsm_between - ln2) < 0.005`,
  condition 2 is satisfied regardless of comparative direction.
  The paper must note this outcome explicitly — it supports separability,
  not comparative advantage on between-state distinctiveness.
...
- **Code:** `counterfactual_suite.test_latent_vs_m2_counterfactual_suite()`
- **Required regime length:** Any
