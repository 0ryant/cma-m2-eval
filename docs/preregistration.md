...
| Battery CF win (within) | M2 within > LSM within | Cosine sim to centroid |
| Battery CF win (between) | M2 between > LSM between OR `TIED_AT_CEILING` | See below |
...
- **Measure:** Two conditions, both must hold:
  1. M2 `mean_within_coherence > LSM mean_within_coherence`
  2. M2 `mean_between_margin > LSM mean_between_margin` — OR —
     both margins are within 0.005 of ln(2) ≈ 0.6931 (`TIED_AT_CEILING`)

- **TIED_AT_CEILING rule:** When both architectures use disjoint action
  supports (masked softmax, non-overlapping tactic sets), between-state
  JS saturates at ln(2) by construction — it measures "disjointness
  exists", not "family distinctness". If both margins are within 0.005
  of ln(2), the between condition is treated as structurally
  non-discriminative. Win is decided on within-state coherence alone.
  This outcome must be reported in the paper as: "Between-state JS
  reached the structural ceiling (ln 2) for both architectures due to
  disjoint action support geometry. This demonstrates separability
  exists; it is not comparative evidence for M2."

- **Win interpretation under TIED_AT_CEILING:** A counterfactual win
  under this condition supports the claim that M2 tactic sets produce
  more stable within-state action distributions than LSM k-means
  clusters, but does NOT support a claim about cross-family
  distinctiveness. Paper framing must reflect this.
